  ...
  return (
    <LegalPage title="特定商取引法に基づく表記" description="Staffpassのオンライン申込みおよび継続課金に関する販売条件です。主として法人・団体・個人事業主向けですが、適用される強行法規上の権利を妨げません。" identity={identity}>
      <div className="table-scroll"><table><tbody>
        ...
        <tr><th>販売価格・役務の対価</th><td><ul><li>スターター: 月額12,000円（税込）</li><li>ビジネス: 月額39,800円（税込）</li><li>Managed: 月額128,000円（税込）</li><li>Business初回導入: 150,000円（税込、表示・選択された場合のみ）</li><li>キックオフパック: 398,000円（税込、任意・別途申込み）</li></ul><p className="mt-2 text-xs faint">現在の画面表示価格は事業確定前の設定を含みます。実際の契約価格は、お支払い手続きの最終確認画面、見積書または注文書に表示された金額が優先します。価格確定前に課金することはありません。</p></td></tr>
        ...
        <tr><th>支払時期</th><td>月額料金は、お支払い手続きの最終確認画面に表示された初回請求日以降、毎月同日に請求します。初期費用・任意パックは申込時または個別に合意した期日に請求します。銀行振込は請求書記載の期日までにお支払いください。</td></tr>
        <tr><th>役務の提供時期</th><td>アカウントおよびOrgは、登録完了後直ちに利用できます。有料機能は決済または契約手続の完了後に提供します。Managed、初回導入およびキックオフ作業の開始・完了時期は個別に調整します。</td></tr>
        <tr><th>トライアル</th><td>標準14日間。初回登録時にカード情報は不要で、カード未登録のまま自動課金されません。有料申込み時の無料期間、初回請求日および条件はお支払い手続きの最終確認画面でご確認ください。</td></tr>
        <tr><th>契約期間・自動更新</th><td>月額プランは1か月単位で、解約されるまで自動更新されます。個別の注文書・見積書に期間の定めがある場合は、その定めが優先します。</td></tr>
        <tr><th>解約方法</th><td>次回更新前に、管理画面の請求管理ページまたは {identity.contactEmail} への電子メールで解約をお申し出ください。解約は原則として現在の支払済み期間の終了時に効力を生じ、解約手数料はありません。</td></tr>
