...
const FAQ: FaqItem[] = [
  ...
  },
  {
    ...
    a: "入れないままだと、AIは個人のSlackやメールに直接つながりがちです。誰の名義で返したか、どこまで許したか、あとから追いにくい。Staffpassを入れると、AIに社員証が付き、Slackの受信と投稿は会社のアプリ経由になります。社内の返信は方針どおりに進め、社外送信・日程確定・発注は人が見るまで止められます。許可・止め方・記録が会社の台帳に残るのが、入れる価値です。",
  },
  {
  ...
  {
    q: "細かい設定はどこまでできますか？",
    a: "テナントごとに変えられます。会話の本文を手足に全文渡すか、先頭だけにするか、渡さないか。添付は本体か、名前と種類だけか、渡さないか。添付を渡すときに上長の承認を要るかも決められます。契約書やNDAなどはそのまま手足に渡さず、Sealith（機密ファイルを暗号化し、用途と期限を限って渡す仕組み）の転送に切り替える設定もできます。全部を一律に厳しくする必要はなく、チャンネルや分類ごとに決められます。AI社員の許可・止め方・記録はStaffpass、ファイルの暗号化受け渡しはSealith、という役割分担です。",
  },
  {
    q: "Sealithとは何ですか？",
