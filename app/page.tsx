...
const CONTROLS = [
  { icon: "01", title: "権限を分ける", body: "社員ごとに職務と操作範囲を限定。社外メールは下書きまで／人が送る／自動、など出口の止め方も会社が決めます。" },
  { icon: "02", title: "人が止める", body: "初期設定では社外送信・日程確定・発注は人が見るまで進みません。承認したあとも、失効した権限では実行しません。" },
  { icon: "03", title: "記録を残す", body: "目的、承認者、実行結果を一つの監査台帳に残します。秘密の受け渡しは一度きりにし、あとから追える形にします。" },
];
...
const FAQ: FaqItem[] = [
  ...
  },
  {
    ...
    a: "会話の上手さではありません。会社として「この子はここまで」「誰が通したか」を残せるかどうかです。",
  },
  {
    q: "Staffpassを入れないと、何が変わる？",
    a: "入れないままだと、AIは個人のSlackやメールに直接つながりがちです。誰の名義で返したか、どこまで許したか、あとから追いにくい。Staffpassを入れると、AIに社員証が付き、Slackの受信と投稿は会社のアプリ経由になります。社内の返信は方針どおりに進め、初期設定では社外送信・日程確定・発注は人が見るまで進みません。事業者の判断で自動にすることもできます。許可・止め方・記録が会社の台帳に残るのが、入れる価値です。",
  },
  {
    q: "ハーネス（実行環境）でも制御できるのでは？なぜ外に出す？",
    a: "止め方はハーネスにもあります。ただしそれは個人や Bot 向けの安全網です。Staffpass は会社の就業規則と日報です。手足を替えても規則が残る、Bot を分けてもコンピュータは共有なので境界にならない、承認と失効は会社の台帳に残す——説明できる止め方と失効できる社員証は、会社の外に置かないと残らない、というのが外に出す理由です。",
  },
  {
    q: "勝手に送ったり、予定を入れたり、買ったりしない？",
    a: "初期設定では、社外送信・日程の確定・発注は人が見るまで進みません。事業者の判断で、自動にすることもできます。そのときは警告を確認したうえで、責任は事業者にあります。Staffpass の仕事は、与えた権限どおりに動かすことと、あとから追える記録を残すことです。承認を通したあとも、社員証が失効していれば実行しません。",
  },
  {
    q: "メールやSlackごとに、止め方を変えられますか？",
    a: "はい。社員証単位で、どの口・どの出口まで通すかを決められます。たとえば社外メールは下書きまで、社内Slackは方針どおりに進める、といった分け方ができます。初期設定では社外送信・日程確定・発注は人が見るまで進みません。事業者の判断で自動にもできます。電話口への同じルール付けは順次対応です。",
  },
  {
    q: "全部自動にできる？",
  ...
  {
    q: "細かい設定はどこまでできますか？",
    a: "AI社員ごとに変えられます。会話の本文を手足に全文渡すか、先頭だけにするか、渡さないか。添付は本体か、名前と種類だけか、渡さないか。添付を渡すときに上長の承認を要るかも決められます。契約書やNDAなどはそのまま手足に渡さず、Sealith（機密ファイルを暗号化し、用途と期限を限って渡す仕組み）の転送に切り替える設定もできます。全部を一律に厳しくする必要はなく、チャンネルや分類ごとに決められます。AI社員の許可・止め方・記録はStaffpass、ファイルの暗号化受け渡しはSealith、という役割分担です。",
  },
  {
    q: "Sealithとは何ですか？",
