...
| **B. 導入・運用代行** | 速さ・伴走・保守 | 「キックオフ代行／日々の整備」 | Managed＝Care 月額（Business＋伴走） |
| **C. ゲート従量** | 日報が残る・確定した仕事 | 「通した仕事の分」 | `gated_confirm_action` |

## 2. プランコード（固定）

| planKey | 表示 | 仮枠（確定アクション/月） |
|---------|------|---------------------------|
| `starter` | スターター | **50**（仮枠） |
| `business` | ビジネス | **500**（仮枠） |
| `managed` | Managed（Care） | **2000**（仮枠） |

- Stripe Price ID: `STRIPE_PRICE_ID_STARTER` / `_BUSINESS` / `_MANAGED`
- 円額は Dashboard が正。コードに実額を事実として書かない。
- 枠は **仮枠・事業確定前**。UI に「仮枠」と明示。
