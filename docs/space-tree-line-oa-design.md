...
**ステータス:** 設計ドラフト — **課金概算は安藤突合 OK（マージ可）**。**P0 単独ではロックしない。** ロック対象 = **P0 運用 + P1 実装仕様**。八坂待ち = **OA 表示名の最終選択** + **P0+P1 設計ロック GO**  
**更新:** 2026-09-14  
**読者:** 導入伴走・運用・エンジニア
...
| **表示名** | 候補 A/B を併記（§P1-e）。**最終決定は八坂**（P1 付き設計を見て選択）。静かな本人偽装は禁止 |
| **P0 スコープ** | **承認のみ**（実装ほぼ既存）。オペ実証。**P0 単独では設計ロックしない** |
| **P1 スコープ** | OA 会話スタブ — **実装仕様まで本 doc で固定**（設計ロック対象の半分） |
| **P2** | 会話本実装。**別 GO**（本ロック対象外） |

**実装 GO は P0+P1 設計ロック後。** 課金概算は安藤突合 OK。八坂待ち = **表示名の最終選択** + **P0+P1 ロック GO**（P0 だけ先行 GO しない）。
...
- **Telegram:** Space Tree テナントの承認経路からは **無効化または demote**（二重送信の混乱防止）。タイミングは P0+P1 ロック後の実装 GO に合わせる。他テナントは影響なし。

---

## 表示名・プロフィール

P1 節 (e) に候補を集約。**最終決定は八坂**（P1 付き設計を読んで選択）。P0 単独では確定しない。
...
| フェーズ | 内容 | 設計ロック | 実装 GO |
|----------|------|------------|---------|
| **P0** | 承認のみ（Flex + postback）。既存コード + テナント設定・オペ実証 | **P1 とセットでロック**（**P0 単独ロックしない**） | P0+P1 ロック後 |
| **P1** | OA 会話スタブ — text wake / always_human push / webhook ルーティング | **ロック対象**（本 doc の P1 節） | P0+P1 ロック後 |
| **P2** | 会話本実装（reply_policy フル、ファイル、Flex 業務等） | **別 GO** | 別 GO |
...
P0 承認と **同一 OA** で会話の「口」を足す。本節が P1 実装の正本。**P0 単独ではロックしない** — 八坂は本節を含む **P0+P1 一式** を見てロック GO と表示名を決める。
...
#### (b) 承認 Webhook とのイベント分離 — **推奨決定（ロック候補）**
...
|---|------|
| **候補 A** | `安藤（AI窓口）` |
| **候補 B** | `Staffpass｜スペースツリー` |
| **プロフィール一文（案）** | 「AI社員が Staffpass 経由で応答します。重要操作は上長承認があります。」 |

**最終決定は八坂**（本節を見て選択）。静かな本人偽装は禁止。
...
P0 オペ実証用。**P1 実装仕様は本文 P1 節参照 — P0+P1 設計ロック後に実装 GO。** P2 は別 GO。
...
- [ ] Channel secret を控える
- [ ] OA 表示名を決定（**八坂が P1 節を見て A/B から選択**。ロック GO 前は仮置き可）
...
- [ ] ~~LINE 会話アダプタ実装~~ → **P1 仕様は本文 §P1。P0+P1 ロック後に実装 GO**
...
**実装 GO は P0+P1 設計ロック後。** 課金・コスト概算は安藤突合 OK（マージ可）。八坂待ち = **OA 表示名の最終選択**（§P1-e 候補 A/B）+ **P0+P1 設計ロック GO**。**P0 単独ではロックしない。** P2 は別 GO。
