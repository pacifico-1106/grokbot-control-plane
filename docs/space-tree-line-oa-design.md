...
**ステータス:** 設計ドラフト — **課金概算は安藤突合 OK**。設計ロック待ち（**OA 表示名・P0 先行方針は八坂選択待ち**）  
**更新:** 2026-09-14  
**読者:** 導入伴走・運用・エンジニア
...
**実装 GO は設計ロック後**（課金概算は安藤 OK。**表示名・P0 先行は八坂選択待ち**。P0 は既存実装のテナント設定・オペ実証）。
...
| # | 推奨 |
|---|------|
| 1 | 実証 **P0** は **コミュニケーションプラン** で開始 |
| 2 | 上長宛は **1:1 userId** を推奨。**グループは人数倍コスト** — 必要時のみ |
| 3 | 会話 **P1** を足す前に **月 Push 実測** → 超過見込みなら **ライト**へアップグレード |
| 4 | **スタンダード** は会話本格化 or ブロードキャスト用途が出てから |
...
**設計ロック待ち。** 課金・コスト概算は安藤突合 OK。OA **表示名** と **P0 先行**（コミュ開始 / 1:1 上長 / Telegram demote タイミング）は **八坂選択待ち**。確定後に実装 GO。P0 は上記チェックリストによる設定・オペ実証。P1/P2 は別 GO で起票する。
